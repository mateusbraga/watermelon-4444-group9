...
public class Coloring{
	...
	public void colorHeuristic(ArrayList<seed> seedList) {
		//System.out.println("Enter colorHeuristic");
		init();
		calMatrix(seedList);
		int n = seedList.size();
		int iter = 20;
		...
		while(iter-- > 0)
		{
			for(int i = 0; i < n; i++)
			{
				//System.out.println("i: " + i + ", before: " + subset[i]);
				subset[i] = !subset[i]; // change the label of i
				double tmp = sum_w;
				for(int j = 0; j < n; j++)
				{
					if(i != j)
					{
						if(subset[j] != subset[i]) // i and j are in different classifications now
						{
							tmp += w[j][i];
							tmp += w[i][j];
						}
				...
				//System.out.println("tmp: " + tmp + ", sum_w: " + sum_w);				
				if(max_w < tmp)
				{
					max_w = tmp;
				    for(int k = 0; k < n; k++)
				    {
				    	best_subset[k] = subset[k];
				    }	
				}
				if(tmp < sum_w)
					subset[i] = !subset[i];
					...
					sum_w = tmp;
				//System.out.println("i: " + i + ", after: " + subset[i]);
